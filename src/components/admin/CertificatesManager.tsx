          ...
          <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 text-slate-100 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold font-mono text-slate-100">Delete Certificate</h3>
                  <p className="text-[11px] text-slate-400 font-mono">Permanent credential removal</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="p-3.5 rounded-xl bg-slate-950/80 border border-slate-800/80 flex items-center gap-3">
              {certToDelete.imageUrl ? (
                <img
                  src={certToDelete.imageUrl}
                  alt={certToDelete.title}
                  className="w-12 h-12 rounded-lg object-cover border border-slate-800 flex-shrink-0"
                />
              ) : (
                <div className="w-12 h-12 rounded-lg bg-emerald-950/40 border border-emerald-800/50 flex items-center justify-center text-emerald-400 flex-shrink-0">
                  <Award className="w-6 h-6" />
                </div>
              )}
              <div className="min-w-0 flex-1">
                <p className="font-bold text-xs text-slate-100 truncate">{certToDelete.title}</p>
                <p className="text-[10px] text-slate-400 font-mono truncate">
                  {certToDelete.issuer} • {certToDelete.category}
                </p>
              </div>
            </div>

            <p className="text-xs text-slate-300 font-mono leading-relaxed">
              Are you sure you want to delete <span className="text-rose-300 font-bold">"{certToDelete.title}"</span>? This will permanently remove it from your portfolio and Cloudinary storage.
            </p>
